...
namespace Panda.SAM {
	...
	public NextSeq? create_reader(string filename, Logger logger, bool binary, uint8 qualmin = 33);
	/**
	 * Create a new assembler for given a SAM file.
	 * @see create_reader
	 */
	[CCode(cname = "panda_assembler_open_sam")]
	public Assembler? open(string filename, owned Logger logger, bool binary, uint8 qualmin = 33);
	/**
	 * Create a new multiplexed reader for given a SAM file.
	 * @see create_reader
	 */
	[CCode(cname = "panda_mux_open_sam")]
	public Mux? open_mux(string filename, owned Logger logger, bool binary, uint8 qualmin = 33);
}
