...
main(
	...
	bool okay;
	const char *optlist = "bB:C:d:f:Fhl:L:No:p:q:Q:t:v"
#ifdef HAVE_PTHREAD
		"T:"
#endif
		;
	...
	if (filename == NULL || help) {
		...
		fprintf(stderr, "[-p forwardprimer] " "[-q reverseprimer] ");
#ifdef HAVE_PTHREAD
		...
		fprintf(stderr, "\t-b\tRead a binary (BAM) file rather than a text (SAM) file.\n");
		fprintf(stderr, "\t-B code\tReplace the Illumina multiplexing barcode stripped during processing into SAM/BAM.\n");
...
#ifdef HAVE_PTHREAD
	mux = panda_mux_open_sam(filename, (PandaLogger) panda_logger_file, stderr, NULL, binary, tag);
	if (mux == NULL) {
...
#else
	assembler = panda_assembler_open_sam(filename, (PandaLogger) panda_logger_file, stderr, NULL, binary, tag);
#endif
	if (assembler == NULL) {
